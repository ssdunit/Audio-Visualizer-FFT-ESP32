...
void updatetext(const char *str, uint8_t *text, int *length){
  *length = 0;
  ...
  while(*str){
    ...
    if(*str >= 'A' && *str <= 'Z'){
      int index = (*str - 'A') * 8; //index give the index of that particular character i.e. B has value of 66 A has 65 so B-A = 1 * 8 so its index is 8
      for(int i=0; i<8; i++){ // loops 8 times as the character has 8 bytes
        text[*length] = font[index+i]; //sets the length index of text as the value of idx+i
        (*length)++;
      }
    }
    str++;
...
void display(){
  ...
  shiftOut(data,srclk,MSBFIRST,lowByte(~matrix[i]));
  shiftOut(data,srclk,MSBFIRST,highByte(~matrix[i]));
